...
func NewHandler(auction *contract.Auction) *Handler {
    return &Handler{auction: auction}
}
...
func (h *Handler) endAuction(c *gin.Context) {
    ...
    if err != nil {
        log.Err(err).Msg("Error occurred while ending the auction")
        c.String(http.StatusInternalServerError, err.Error())
        return
    }
    c.String(http.StatusOK, tx.Hash().String())
...
func (h *Handler) withdraw(c *gin.Context) {
    ...
    if err != nil {
        log.Err(err).Msg("Error occurred while withdrawing")
        c.String(http.StatusInternalServerError, err.Error())
        return
    }
    c.String(http.StatusOK, tx.Hash().String())
...
func (h *Handler) createBid(c *gin.Context) {
    ...
    if err != nil {
        log.Err(err).Msg("Error occurred while placing bid")
        c.String(http.StatusInternalServerError, err.Error())
        return
    }
    c.String(http.StatusOK, tx.Hash().String())
